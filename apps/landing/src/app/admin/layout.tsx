...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Buscar notificações (solicitações pendentes)
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const { data: requests } = await supabase
          .from('access_requests')
          .select('id, salon_name, owner_name, created_at')
          .eq('status', 'pending')
          .order('created_at', { ascending: false })
          .limit(10)

        if (requests && (requests as Array<{ id: string; salon_name: string; owner_name: string; created_at: string }>).length > 0) {
          const typedRequests = requests as Array<{ id: string; salon_name: string; owner_name: string; created_at: string }>
          const notifs: Notification[] = typedRequests.map(req => ({
            id: req.id,
            type: 'request',
