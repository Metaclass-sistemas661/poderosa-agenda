-- ============================================================================
-- Migration: Update Salon Details (Superadmin Enterprise RPC)
-- Date: 2026-08-24
-- Author: Enterprise Security Team
--
-- RPC for updating salon details with full audit trail.
-- Only superadmins can execute this function.
-- Records old vs new values in audit_logs for compliance.
-- ============================================================================
...
BEGIN
    ...
    -- 4. Apply updates (only non-null parameters)
    UPDATE salons SET
    ...
    IF v_old_record->>'name' IS DISTINCT FROM v_new_record->>'name' THEN
        v_changes := v_changes || jsonb_build_object('name', jsonb_build_object('old', v_old_record->>'name', 'new', v_new_record->>'name'));
    END IF;
    IF v_old_record->>'owner_name' IS DISTINCT FROM v_new_record->>'owner_name' THEN
        v_changes := v_changes || jsonb_build_object('owner_name', jsonb_build_object('old', v_old_record->>'owner_name', 'new', v_new_record->>'owner_name'));
    END IF;
    IF v_old_record->>'email' IS DISTINCT FROM v_new_record->>'email' THEN
        v_changes := v_changes || jsonb_build_object('email', jsonb_build_object('old', v_old_record->>'email', 'new', v_new_record->>'email'));
    END IF;
    IF v_old_record->>'phone' IS DISTINCT FROM v_new_record->>'phone' THEN
        v_changes := v_changes || jsonb_build_object('phone', jsonb_build_object('old', v_old_record->>'phone', 'new', v_new_record->>'phone'));
    END IF;
    IF v_old_record->>'plan' IS DISTINCT FROM v_new_record->>'plan' THEN
        v_changes := v_changes || jsonb_build_object('plan', jsonb_build_object('old', v_old_record->>'plan', 'new', v_new_record->>'plan'));
    END IF;
    IF v_old_record->>'status' IS DISTINCT FROM v_new_record->>'status' THEN
        v_changes := v_changes || jsonb_build_object('status', jsonb_build_object('old', v_old_record->>'status', 'new', v_new_record->>'status'));
    END IF;
...
COMMENT ON FUNCTION update_salon_details IS 'Updates salon details with full audit trail. Superadmin only. Records old vs new values.';
